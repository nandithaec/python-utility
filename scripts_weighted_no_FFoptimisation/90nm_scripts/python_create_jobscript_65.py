...
if os.path.isfile('jobscript.txt'):
...
fw.write('python %s/%s -m %s -p %s -d %s -t %s -n %s --group %s --clk %s  %s >/dev/null 2&>1\n' %(path,script,module,path,design_folder,tech,num,num_at_a_time,clk,))
