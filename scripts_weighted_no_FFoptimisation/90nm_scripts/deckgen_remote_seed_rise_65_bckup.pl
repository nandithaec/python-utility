#Example: perl deckgen_remote_seed_rise_65.pl -s reference_spice.sp  -r c432_clk_ipFF_reference_out/tool_reference_out.txt -n 1 -m c432_clk_ipFF -f /home/users/nanditha/Documents/utility/90nm/c432 -g 4 -d 2 -c 200 -i 4.91e-09 -o 1 

#Modifications:
#added next_3_cycle for 3rd edge and 4th edge measurements for 3 cycle simulation: Mar 26 2014
#added parameter $vdd=1; #for 90nm technology. This is because the current script ppicks $vdd from reference.sp file matching the Vvdd string, which is not present in the  90nm ref file. because, its name is vcc and not vdd: Mar 21 2014
#Removed the option of reading the $lib file : not needed: Mar 19 2014
...
deckgen.pl will modify the template spice file to introduce the glitched version of the gate in the spice file. The deckgen will get the 4 random numbers as input: random clk, random glitch instant, random gate and the random drain. It will go to the corresponding clk cycle in the c432_clk_ipFF_reference_out/tool_reference_out.txt and grab the input values at that clk edge and will insert these values into the reference_spice.sp file, to create, say, deck_1.sp. Again, for the next set of random numbers, the same is repeated, till the max number of decks specified, is reached. For each of the random clk cycles picked, this script will also write out the corresponding output values (which were written on the -ve edge) of the next clock cycle, into the c432_clk_ipFF_reference_out/RTL.csv file, which will be used as a reference value, during comparison of spice vs verilog simulation.
 ...
 {  
    ...
    if($count==1)
 	 ...
 	 $minus4_cycle =  $minus3_cycle;
         ...
         $next_2_cycle=$next_3_cycle;
         chomp($next_3_cycle=$_);
       }
	   ...
	   ($temp1,$pinname)=split(" ",$_);
           ...
           foreach $index( 0 .. $#temp)
			...
			#This is for the PWL statements in the spice file
		      ...
		      print "cycle_minus1[$index]: $cycle_minus1[$index]\n";
		      $ref_minus1=$cycle_minus1[$index]*$vdd;
		      print "ref_minus1:  $ref_minus1\n";
...
print IM "\n";
print "Random drain..RTL_2nd_edge.csv  ----- $random_drain\n";
#A comma is needed at the end of the last entry
print IM "$deck_num,$cycle,$glitch_location,$random_gate,$rand_gate,$random_drain,";
