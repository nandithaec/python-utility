...
#Example usage: python python_utility3_remote_seed_yuva_echo.py -m c432_clk_opFF -p /home/nanditha/Documents/iitb/utility/c432_priority_opFF -d c432_priority_opFF -t 180 -n 2 --group 2 --clk 100 
import optparse
import re,os
...
#########################################repeat_deckgen copied starting from here#######################################
	...
	for loop_var in range(start, end+1): 
		...
		os.system('perl %s/perl_calculate_drain_65.pl -s %s/reference_spice.sp -r %s/%s_reference_out/tool_reference_out.txt -m %s -f %s -g %d ' %(path,path,path,module,module,path,rand_gate))
