...
#Nanditha Rao
#Example usage: python python_FF_strike_taxonomy.py  -p /home/external/iitb/nanditha/simulations/decoder_ip_opFF_rise -d decoder_ip_opFF_rise

def taxonomy_FFs(path,subdirectory):
	...
	parser.add_option("-d", "--design", dest="design",help="Enter the name of the current design ")

	(options, args) = parser.parse_args()

	path=options.path
	subdirectory=options.design
	...
	#Now, write out the 2nd edge and 3rd flip summary for gates into one file.. and identify the type of fault
		...
		for aa in range(5,len(headers3)):  
		...
		for row in FF_read_2:
			...
			#The 1st 5 values are already appended. They contain,clk num, deck num,gate name etc
			for aa in range(5,len(row3)):  
		...
		#Now, append the type of fault to the same row..
			if (int(row[inputFF_2nd_rise[0]])==0 and int(row3[outputFF_3rd_rise[0]])==0):
				...
				if (int(row[outputFF_2nd_rise[0]])>1):
					row.append("Not possible to affect more than 1 FF output by a direct strike")
				

				elif (int(row[outputFF_2nd_rise[0]])==1):
					row.append("FF output flipped by a direct strike")
					FF_output_glitch=FF_output_glitch+1
	###########################################
		...
		print "Output written: %s/spice_results/taxonomy_summary_FFs_%s.csv" %(path,subdirectory)
		...
		fout = open('%s/spice_results/taxonomy_summary_FFs_%s.csv' %(path,subdirectory), 'wb')
		fout.write ("\n*****************************%s results**********************************\n" %subdirectory)
		...
		doc1 = SimpleDocTemplate("%s/spice_results/taxonomy_report_FFs_%s.pdf" %(path,subdirectory), pagesize=letter)
		...
		elements.append(p)
		p = Paragraph('''Design: %s''' %subdirectory, style)
		elements.append(p)
		...
		print "%s/spice_results/taxonomy_report_FFs_%s.pdf has the results." %(path,subdirectory)
		doc1.build(elements)
