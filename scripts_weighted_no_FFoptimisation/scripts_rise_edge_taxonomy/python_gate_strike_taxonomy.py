...
#Nanditha Rao
#Example usage: python python_gate_strike_taxonomy.py  -p /home/external/iitb/nanditha/simulations/decoder_ip_opFF_rise -d decoder_ip_opFF_rise

def taxonomy_gates(path,subdirectory):
	...
	#Now, write out the 2nd edge and 3rd flip summary for gates into one file.. and identify the type of fault
		...
		#Now, append the type of fault to the same row..
			...
			if (int(row[inputFF_2nd_rise[0]])==0 and int(row3[outputFF_3rd_rise[0]])==0):
				print "case 1"
				temp_row.append("Glitched and masked (did not flip any FF)")
		...
		print "Output written: %s/spice_results/taxonomy_summary_gates_%s.csv" %(path,subdirectory)
		...
		fout = open('%s/spice_results/taxonomy_summary_gates_%s.csv' %(path,subdirectory), 'wb')
		fout.write ("\n*****************************%s results**********************************\n" %subdirectory)
		...
		doc1 = SimpleDocTemplate("%s/spice_results/taxonomy_report_gates_%s.pdf" %(path,subdirectory), pagesize=letter)
		...
		print "%s/spice_results/taxonomy_report_gates_%s.pdf has the results." %(path,subdirectory)
		doc1.build(elements)
