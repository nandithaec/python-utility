...
#Example usage: python python1_read_RTL_syn_pnr_65.py -f /home/users/nanditha/Documents/utility/65nm/LFSR/lfsr.vhd -m lfsr -c 400 -idelay 1.1 --odelay 0.7 --ipff iDFF --opff oDFF -p <path>
...
#Remove clock gating
#Modify case-insensitive net names
for line in fin:
	if line == 'set_clock_gating_style  \\n':
		...
		next_line = next(fin) #store next line
        	if next_line == 'compile -map_effort medium\n':
        		fnew.write('set_dont_retime [get_cells "%s*"]\nset_dont_retime [get_cells "%s*"]\n' %(ip_FF,op_FF))
        		print('set_dont_retime [get_cells "%s*"]\nset_dont_retime [get_cells "%s*"]\n' %(ip_FF,op_FF))
        		print('%s_*\nset_dont_retime [get_cells %s*]\n' %(ip_FF,op_FF))
	...
	elif line == 'compile -incremental_mapping -map_effort medium\n':
...
print "Eliminating clock gating.. optimizing flip-flop placement\n"
