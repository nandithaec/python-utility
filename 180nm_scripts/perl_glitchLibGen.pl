#Example: perl GlitchLibGen.pl -i osu018_stdcells_correct_original.sp -p <path>
...
#Removed #($dlist=~m/$drain/)|| option - Oct 21 2014
           ...
           foreach $i (0 .. $#sub_ckt)
	     {
	       ...
	       if($sub_ckt[$i]=~m/^M\d* /)
		    ...
		    ($temp,$drain)=split(' ',$sub_ckt[$i]);
		     ...
		     if(!(($drain=~m/gnd/)||($drain=~m/vdd/i)||($drain=~m/0/))) #($dlist=~m/$drain/)||
		       {			                   
			$dlist.=" $drain";
		        $drain_num++;
