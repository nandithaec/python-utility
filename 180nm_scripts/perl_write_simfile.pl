...
open(REF_NEG,">$path/$ref/our_reference_out_neg.txt")||die("unable to open file : $!");
...
open(TOOLREF_NEG,">$path/$ref/tool_reference_out_neg.txt")||die("unable to open file : $!");
...
#Adding reference output generating statements
	...
	print OPT 'fileout_neg= $fopen("'.$path.'/'.$ref.'/our_reference_out_neg.txt","a+");'."\n";
	print OPT 'fileout1_neg= $fopen("'.$path.'/'.$ref.'/tool_reference_out_neg.txt","a+");'."\n";
...
print "\t\t\t\t\t*                      *\n";
print "\t\t\t\t\t************************\n";
print "***MODELSIM simulatable file \" $path/$op \" created in the current directory\n";
