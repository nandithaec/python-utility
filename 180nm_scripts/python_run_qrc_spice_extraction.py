...
#Added the path to the output file -file_name - Oct 21 2014
...
##Example usage: python python2_run_qrc_spice_extraction.py -m decoder_op_ip -p <path>
...
fw = open('%s/qrc_dspf.cmd' %path, 'w') ## This is the commands input file for Cadence QRC- RC extraction
...
fw.write('\t -design_file "%s/pnr/op_data/%s_final.def.gz" \\\n' %(path,module))
...
#Run the ccl script
os.system('qrc -cmd %s/qrc_dspf.cmd' %path)	
print "\n****Completed generating the spice (dspf) netlist in the current working directory.****\n"
print "Spice file name: %s/%s.dspf" %(path,module)
