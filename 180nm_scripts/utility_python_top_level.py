...
parser.add_option("-p","--path", help='Enter the entire path to your design folder',dest='path')
#########################################################################
...
parser.add_option("--extl", dest="folder",help="Enter the ENTIRE path to your design folder (your working dir)- either this machine or remote machine where simulations will be run. The name of the folder there should be the same as the name of the folder being copied from the current machine. IF remote machine, enter /home/user1/simulations/<design_folder_name>")
#########################################################################
...
os.system('perl perl_glitchLibGen.pl -i osu018_stdcells_correct_vdd_gnd.sp -p %s' %(main_path))
...
os.system('python python_create_jobscript.py -m %s -p %s -d %s -t %s -n %s --group %s --clk %s   --proc_node %s --ppn %s --days %s --hrs %s --mins %s --script %s --path_here %s  --scripts_path %s' %(module,path_folder,design_folder,techn,num,group,clkfreq,nodes,ppn,days,hrs,mins,script,main_path,scripts_path))
time.sleep(5)
