...
#Eliminated std_lib option - Oct 18 2014
...
#Example usage: python python_utility_step2_yuva.py -m c499_clk_ipFF -p /home/nanditha/Documents/iitb/utility/c499 -d c499 -t 180 -n 2 --group 2 --clk 100 --scripts_path /home/external/iitb/nanditha/simulations/180nm/scripts_run/
...
os.system('perl %s/perl_calculate_gates_clk.pl -s %s/reference_spice.sp  -r %s/%s_reference_out/tool_reference_out.txt -m %s -f %s ' %(scripts_path,path,path,module,module,path))
...
#########################################repeat_deckgen copied starting from here#######################################
	...
	fout = open('%s/%s_reference_out/RTL_2nd_edge.csv' %(path,module), 'w')
	fin = open('%s/%s_reference_out/RTL_backup.csv' %(path,module), 'r')
	...
	for loop_var in range(start, end+1): 
		...
		os.system('perl %s/perl_calculate_drain.pl -s %s/reference_spice.sp -l %s/glitch_osu018_stdcells_correct_vdd_gnd.sp -r %s/%s_reference_out/tool_reference_out.txt -m %s -f %s -g %d ' %(scripts_path,path,path,path,module,module,path,rand_gate))
...
#This means, glitch "can" occur before the input changes in the clk period as well. So, force the glitch to start only after input has changed
		...
		rand_glitch= (0.51*clk_period) +  unif #arrival_clk + initial_clk should add up to 1.5
...
#perl deckgen_remote_seed.pl -s reference_spice.sp -l glitch_osu018_stdcells_correct_vdd_gnd.sp -r decoder_behav_pnr_reference_out/tool_reference_out.txt -n 1 -m decoder_behav_pnr -f /home/user1/simulations/decoder -g 27 -d 2 -c 10 -i 1.42061344093991e-09 -o 1 
		...
		os.system('perl %s/perl_deckgen.pl -s %s/reference_spice.sp  -r %s/%s_reference_out/tool_reference_out.txt -n %d -m %s -f %s  -o %s -g %s -d %s -c %s -i %s' %(scripts_path,path,path,module,loop_var,module,path,loop,rand_gate,rand_drain,rand_clk,rand_glitch))

##################Script repeat_deckgen copied ends here####################################
	
	
	print "Running GNU Parallel and ngspice on the created decks\n"
	os.system('python %s/python_GNUparallel_ngspice_yuva.py -n %s -d %s -o %s -p %s' %(scripts_path,num_at_a_time,design_folder,loop,path))
	...
	os.system('python %s/python_compare_3rd_rise.py -m %s -f %s -n %s -t %s -l %d' %(scripts_path,module,path,num_at_a_time,tech,loop))

	print "Comparing the RTL and spice outputs at the 2nd rising edge \n"
	os.system('python %s/python_compare_2nd_rise.py -m %s -f %s -n %s -t %s -l %d' %(scripts_path,module,path,num_at_a_time,tech,loop))
...
shutil.copy('%s/glitch_osu018_stdcells_correct_vdd_gnd.sp' %(path), '%s/backup_spice_decks_3rd_edge' %path )
shutil.copy('%s/tsmc018.m' %path, '%s/backup_spice_decks_3rd_edge' %path )

#For validation of backup spice files
#Copy the lib sp file and the technology (*.m) file to the backup directory
shutil.copy('%s/glitch_osu018_stdcells_correct_vdd_gnd.sp' %(path), '%s/backup_spice_decks_2nd_edge' %path )
shutil.copy('%s/tsmc018.m' %path, '%s/backup_spice_decks_2nd_edge' %path )

print "Combining all rtl diff files\n"
os.system('python  %s/python_count_flips_2nd_3rd_rise.py -f %s  -n %s  --group %s -s %s' %(scripts_path,path,num,num_at_a_time,seed))  #To save the seed to results file
