#!/usr/bin/perl
#Example usage: perl modperl2_outwrtr_rise.pl -v pnr/op_data/b09_mod_final.v -m b09_mod

#Modifications:
#Code being modified for DFFSR outputs. It had only DFF outputs. Modelsim fwrite statements and RTL.csv headers will change
...
while(<VLOG>)
 {  
   ...
   if(($_=~m/module $module \(/ .. /endmodule/)) #parsing the main module only
     {
	 $in_main_module=1;
	 #Code does not cover LATCH
	 if(($_=~m/DFFPOSX1/)||($_=~m/DFFNEGX1/)||($flag==1)) #searching for D flip flops
	    {
		if($flag == 0)
		   {
	    ...
	    {
	    	if(($_=~m/DFFSR/)) #searching for SR FFs
	    	{
	    		$dffsrflag=1;
	    		print "DFF line: $_\n";
	    	}
	    		...
	    		if($flag == 0)
		   	{
				...
				($fftype,$ffname,$pin)=split(" ",$_);#capturing the output pin of the flip flop
                     		print "FFtype: $fftype, FF name $ffname, Pin $pin \n";
		    		# $pin=~m/\(.*\((.*)\)/;
         	       		# print "Pin changed: $1 \n";
         	        	#my @data = split(/\|/, $line);
	             		#my $nextline = <VLOG>; #2nd line
	             		#my $nextline = <VLOG>; #3rd line
	             		#print "...$_ \n";
	           		if($dffsrflag == 1 && ($_=~m/Q\((.+)\)/))
		 		{
		    			print "Match1\n" ;
		   	
	            		# ($pin1)=split(" ",$nextline);#capturing the output pin of the flip flop
	         		#    if(($nextline=~m/.Q\(/))
	          		#   {
