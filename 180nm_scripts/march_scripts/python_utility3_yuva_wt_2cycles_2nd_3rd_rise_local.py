...
#Example usage: python python_utility3_yuva_wt_2cycles_2nd_3rd_rise_local.py -m lfsr -p /home/nanditha/Documents/simulations/yuva/LFSR -d  LFSR -t 180 -n 8 --group 8 --clk 320 --std_lib osu018_stdcells_correct_vdd_gnd.sp
...
#perl deckgen_remote_seed.pl -s reference_spice.sp -l glitch_osu018_stdcells_correct_vdd_gnd.sp -r decoder_behav_pnr_reference_out/tool_reference_out.txt -n 1 -m decoder_behav_pnr -f /home/user1/simulations/decoder -g 27 -d 2 -c 10 -i 1.42061344093991e-09 -o 1 
		...
		os.system('perl %s/deckgen_remote_seed_rise.pl -s %s/reference_spice.sp -l %s/glitch_%s -r %s/%s_reference_out/tool_reference_out.txt -n %d -m %s -f %s  -o %s -g %s -d %s -c %s -i %s' %(path,path,path,std_lib,path,module,loop_var,module,path,loop,rand_gate,rand_drain,rand_clk,rand_glitch))
...
################################################################################################################
	...
	os.system('python %s/python_GNUparallel_ngspice_rise_local_mc.py -n %s -d %s -o %s -p %s' %(path,num_at_a_time,design_folder,loop,path))
	...
	os.system('python %s/python_compare_3rd_rise.py -m %s -f %s -n %s -t %s -l %d' %(path,module,path,num_at_a_time,tech,loop))

	print "Comparing the RTL and spice outputs at the 2nd rising edge \n"
	os.system('python %s/python_compare_2nd_rise.py -m %s -f %s -n %s -t %s -l %d' %(path,module,path,num_at_a_time,tech,loop))
...
os.system('python %s/python_combine_pdfs_local.py -p %s/spice_results -m %s' %(path,path,module))
