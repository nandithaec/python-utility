...
parser = OptionParser("This script reads in the spice_results/spice_rtl_difference_summary.csv and count_flips_summary.csv for both 2nd edge and 3rd edge. It identifies rows that had strike on a gate and strike on flip-flop and classifies them accordingly. These files are written out:\n1. <path>/spice_results/strike_on_gates_taxonomy.csv and\n2.<path>/spice_results/strike_on_FF_taxonomy.csv \n 3. strike_on_gates_2nd_edge_count.csv and strike_on_gates_3rd_edge_count.csv.4.taxonomy_report_gates_<design>.pdf The flips for 'strike on gate' are further classified into into NN, NF, FN, FF where N means 'no-flip', F means 'flip' in the 2 clock cycles.The flips along with this classification is written out in strike_on_gates_taxonomy.csv. The final table containing the same results is written out in spice_results/taxonomy_report_gates_<design>.pdf \n author- Nanditha Rao: nanditha(at) ee.iitb.ac.in\n")
