...
#Example usage: python python_compare_3rd_edge.py -m decoder_op_ip -f /home/external/iitb/nanditha/simulations/decoder_ip_opFF_rise -n 10 -t 180 -l 1
...
frtl = open('%s/%s_reference_out/RTL_%d.csv' %(path,module,int(outloop)), 'rb')
...
fd = open('%s/spice_results/spice_rtl_difference_3rd_edge_%d.csv' %(path,int(outloop)), 'rb') 
diff_file = csv.reader(fd)
...
#shutil.copytree('%s/spice_decks' %path, '%s/backup_spice_decks_3rd_edge/spice_%d' %(path,loop),symlinks=False, ignore=None)
#Save decks in which > 4 flips occured
if len(more_than4_flips) != 0 :
	
	len_morethan4=len(more_than4_flips)
	if not os.path.exists('%s/backup_spice_decks_3rd_edge/four_flip' %(path)):
		os.mkdir('%s/backup_spice_decks_3rd_edge/four_flip' %(path))
...
if len(three_flips) != 0 :
	...
	if not os.path.exists('%s/backup_spice_decks_3rd_edge/three_flip' %(path)):
		os.mkdir('%s/backup_spice_decks_3rd_edge/three_flip' %(path))
...
if len(two_flips) != 0 :
	...
	len2=len(two_flips)
	if not os.path.exists('%s/backup_spice_decks_3rd_edge/two_flip' %(path)):
		os.mkdir('%s/backup_spice_decks_3rd_edge/two_flip' %(path))
...
if len(one_flip) != 0:
	...
	len1=len(one_flip)
	if not os.path.exists('%s/backup_spice_decks_3rd_edge/one_flip' %(path)):
		os.mkdir('%s/backup_spice_decks_3rd_edge/one_flip' %(path))
...
if len(no_flip) != 0:
	...
	len0=len(no_flip)
	if not os.path.exists('%s/backup_spice_decks_3rd_edge/no_flip' %(path)):
		os.mkdir('%s/backup_spice_decks_3rd_edge/no_flip' %(path))
