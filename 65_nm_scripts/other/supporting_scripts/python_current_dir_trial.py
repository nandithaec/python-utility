...
#Modifying the current parameter depending on whether it is PMOS or NMOS current injection. Also, the direction of current changes depending on whether it is NMOS or PMOS - Aug 11 2014
#Added time=0 measurements= Jul 10 2014
...
for i in range(0,num_of_files):
	...
	data=fin.readlines()
	s=data[0] #1st line of the spice file	
	#Check if the node in which the glitch is injected is PMOS or NMOS	
	#Search for the string between the 2 strings random_gate and random_drain on that line
	result=re.search("random_gate_(.*)random_drain",s)
	gate=result.group(1) #Get the name of that gate  
	#Get the drain number
	result=re.search("random_drain_(.*)_deck",s)
	drain=result.group(1)
	gate_drain=gate+drain #concatenate
	
	#Search for the subckt in the glitched library
	fgl= open('%s/hspice_glitch_CORE65GPSVT_selected_lib_vg.sp' %(path), 'r') 	
	gl_data=fgl.readlines()
	...
	for line in fin:
		...
		elif ".param current_magnitude" in line:
			if pmos==1:
				fnew.write("**PMOS current=NMOS current/3 \n.param current_magnitude = 0.3mA")
			else:
				fnew.write("**PMOS current= - NMOS current\n.param current_magnitude = -1.1mA")	
