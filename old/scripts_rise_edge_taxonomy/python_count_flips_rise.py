...
fd = open('%s/spice_results/spice_rtl_difference_1.csv' %(path), 'r') #Open in read mode: To read the header
...
##############################Combine all the spice_rtl_difference_rise_*.csv files into one file#############################
fw2 = open('%s/spice_results/spice_rtl_difference_rise_summary.csv' %(path), 'w')
summary_writer = csv.writer(fw2)
fd2 = open('%s/spice_results/spice_rtl_difference_rise_1.csv' %(path), 'r') #Open in read mode: To read the header
...
#for the number of files that exist
for num in range(1,(num_of_loops+1)): 
	fr2 = open("%s/spice_results/spice_rtl_difference_rise_%d.csv" %(path,num), "r")
...
fd_rise = open('%s/spice_results/spice_rtl_difference_rise_summary.csv' %path, 'rb') #Open in read mode
...
#column iteration - number of columns. Dont count first 5 columns- since they are deck_num, clk, glitch, gate and subcktlinenum
	...
	for i in (range(5,num_col)): #python will stop looping at (num_col - 1)
		...
		k.append(row[i]) #append the next row in the diff_file
		count_num_fall = count_num_fall + int(row[i])
		
	#Next append all the diff_rise elements in the same row
	...
	#for i in (range(5,last_col)): #python will stop looping at (last_col - 1)	
