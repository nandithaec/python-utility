...
#Example usage: python python_repeat_deckgen_remote.py -n 20 -m decoder_behav_pnr -f /home/user1/simulations/decoder -s 2323
...
for loop_var in range(start, end+1): 
	#x= random.seed(seed*loop_var)
	#print "Random seed is ", x
	seed_mod= int(random.randrange(100000)*random.random())  #Some random number seed for deckgen script
	print "Random num which becomes seed in main script is ", seed_mod


	os.system('perl %s/deckgen_remote_seed.pl -s %s/reference_spice.sp -l %s/glitch_osu018_stdcells_correct_original.sp -r %s/%s_reference_out/tool_reference_out.txt -n %d -m %s -f %s -e %s -o %s' %(path,path,path,path,module,loop_var,module,path,seed_mod,outloop))
