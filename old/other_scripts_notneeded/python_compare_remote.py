...
#Example usage: python python_compare_remote.py -m decoder_behav_pnr -f /home/user1/simulations/decoder -n 15 -t 180 -l 10
...
###Count the number of flips and backup the corresponding sp files#########
...
##########Backup 0.5% of the total spice files#########
loop=int(outloop)
...
if len(two_flips) != 0 :
	...
	if len2 > 4:
		len2=4 #Save only 4 decks in a single run, if there are many double flips. Since we are running few sims on desktop m/c, hope is that, if there are multiple flip decks on that m/c, we wont be able to back it up. Hence out of 3 m/cs, atleast few will be saved. Hence increasing len2 from 2 to 4
