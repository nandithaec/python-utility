...
if ($#ARGV >= 0 || $ref eq "" || $spc eq ""|| $lib eq "" || $deck_num eq "" || $module eq "") {
...
print "the random gate selected is gate nummber $j \n";
...
`mkdir spice_decks`;
open(OPT,">./spice_decks/deck_$deck_num")||die("unable to open file : $!");
...
#Substituting the deck number
     if($_=~m/##deck_num##/)
        {
         $new='echo "$&output_dec_7__voltage" , "$&output_dec_6__voltage" , "$&output_dec_5__voltage" , "$&output_dec_4__voltage" , "$&output_dec_3__voltage" , "$&output_dec_2__voltage" , "$&output_dec_1__voltage" , "$&output_dec_0__voltage" , > glitch_report_outputs_'.$deck_num.'.csv  $$New file';
        }
