...
#Changed "DFF" to "DF" to facilitate the same script to work for both DFF and DFP names for the flip-flops. q_reg option added, to identify ISCAS and ITC benchmarks- and identify Q or q node names respectively - Nov 30 2014

#Example: python python_xors_DFF_output.py -p /home/users/nanditha/Documents/utility/65nm/LFSR -m lfsr
import re,time,os
...
for i in range(len(lines)):
	...
	if q_reg==1: #ISCAS
		...
		if re.search("DFF",lines[i]): #This is for iscas
			...
			elif words[2].startswith("(.q("): #3rd word is the Q node- capture the output node name- for iscas
				dff_out=words[2][5:]
