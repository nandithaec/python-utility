...
parser.add_option("-b", "--dut",dest='dut', help='Enter the name of the DUT which is instantiated within the testbench, to which the entity is bound with the component.')
...
#This will eventually become a loop variable
#row_number=139
#################################Run for all rows##########################################
for row_number in range(0,len(summary)):
	...
	sim_time=(int(clk_cycle)*4000)-1800 #So many picoseconds minus slightly less than half clk cycle
	...
	if row_number==0: #Delete this work directory only first time.. else retain it as it is
		os.system('python F0_python3_create_simdo_vsim_65_1.py -v F0_%s_modelsim.v -t %s -b %s -r %dps -p %s -d %s -m %s -f %s' %(module,test_path,test_module,sim_time,path,dutname,module,flipped_output))
	else:
		os.system('python F0_python3_create_simdo_vsim_65.py -v F0_%s_modelsim.v -t %s -b %s -r %dps -p %s -d %s -m %s -f %s' %(module,test_path,test_module,sim_time,path,dutname,module,flipped_output))
