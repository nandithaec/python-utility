...
fw.write('\n\n#vlib work \n') #Not needed when library already exists. Else, modelsim will run with a warning
...
for a in range(len(ff_headers)-1):
	...
	fdo.write("add wave {sim:/%s/%s/%s/Q}\n" %(test_module,dut,ff_names))
	print "add wave {sim:/%s/%s/%s/Q}\n" %(test_module,dut,ff_names)
...
fdo.write('run 4ns\n') #This is because the clock period in the test bench was of 4000ps duration
...
fdo.write('run 6ns\n') #This is because the clock period in the test bench was of 4000ps duration. Runningfor half clock cycle is good enough
...
fw1.write('\\rm -rf %s/work/%s/*.*\n' %(path,module))
fw1.write('\\rm -rf %s/work/%s/*.*\n' %(path,test_module))

#Run simulation, do not invoke modelsim GUI
#fw1.write('vsim -c -do simulate_vsim.do >vsim.log\n')

#Run simulation, invoke modelsim without the GUI
fw1.write('vsim -c -do %s/F0_simulate_vsim.do \n' %path)
